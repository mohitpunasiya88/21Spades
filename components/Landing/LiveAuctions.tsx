...
export default function LiveAuctions() {
  ...
  // Parse epoch timestamp - handles multiple formats
  const parseEpochTimestamp = useCallback((value: unknown): number | undefined => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        let liveAuctions = nftsData.filter((nft: any) => {
          ...
          if (auctionType === 2) {
            // Use proper parsing function - try multiple field names
            const startingTime = parseEpochTimestamp(nft.startingTime || nft.startTime || nft.start || nft.auctionStartTime);
            const endingTime = parseEpochTimestamp(nft.endingTime || nft.endTime || nft.end || nft.auctionEndTime || nft.auctionEnd);
  ...
  const mappedAuctions = useMemo(() => {
    ...
    return mapped;
  }, [apiAuctions, calculateTimeLeft, timeUpdate]);
