...
function AuthLeftPanel() {
  return (
    <div className="hidden md:block w-1/2 relative px-5 overflow-hidden bg-black">
      {/* Bottom-right purple glow to match signup page */}
      <div className="absolute bottom-0 right-0 w-64 h-64 md:w-96 md:h-96 bg-[#480C64] rounded-full opacity-30  pointer-events-none"></div>
      ...
      <div className="absolute top-0 left-0 right-0 w-full h-full flex justify-center gap-6 p-6">
        <div className="flex flex-col gap-6 animate-scroll-up">
          {[...cardImages, ...cardImages].map((src, index) => (
            <div
              key={`col1-${index}`}
              className={`w-64 h-96 rounded-3xl flex-shrink-0 shadow-2xl relative overflow-hidden`}
            >
